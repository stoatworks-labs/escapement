...
/**
    ...
    Every "random" quantity in Escapement -- a seeded rig's scales, rotations and
    fixed points, and the grain the sensor adds -- comes from here, and the
    reason it is an integer hash rather than the usual
    `fract( sin( x ) * 43758.5453 )` is that the usual one is transcendental. Its
    result differs between GPUs, between drivers, and between a GPU and a CPU.
    Escapement builds its tap sets on the CPU (see Taps.h) so they never have to
    agree with a shader -- but they do have to agree with **themselves on another
    machine**. Seed 4021 is a rig an operator can write on a setlist, and a
    composition built on the show laptop and opened on the rack machine has to
    put that rig's fixed points in the same places. `lowbias32` is exact
    everywhere; a sine is not.
