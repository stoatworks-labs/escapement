...
/**
    ...
    - **The frame store would not allocate.** A float colour attachment is not
      guaranteed renderable, and a rig with no frame store is a rig with no
      loop -- which on screen is an empty clip, exactly like a rig whose gain is
      too low to sustain anything.
    - **A tap was dropped as singular.** Legitimate for Barnsley's stem and a
      sign of a degenerate seeded rig for anything else, and invisible either
      way: the picture simply has one fewer copy in it than the operator asked
      for.
*/
namespace escapement::diag
{
