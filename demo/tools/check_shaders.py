...
PASSES = {
    "kFullscreenVertexShader": "FULLSCREEN_VERTEX_SHADER",
    "kIteratorFragmentShader": "ITERATOR_FRAGMENT_SHADER",
    "kLoopFragmentShader": "LOOP_FRAGMENT_SHADER",
    "kDisplayFragmentShader": "DISPLAY_FRAGMENT_SHADER",
}
...
def cpp_literals(text):
    ...
    pattern = re.compile(
        r'const char\* const (\w+)\s*=\s*R"\((.*?)\)";', re.DOTALL
    )
